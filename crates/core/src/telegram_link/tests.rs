...
fn build_init_data(bot_token: &str, user_json: &str, auth_date: i64) -> String {
    ...
    format!("auth_date={auth_date}&user={}&hash={hash}", urlencoding_minimal(user_json))
}
...
fn identity_persists_across_reloads() {
    ...
    assert_eq!(a.0, b.0, "reloading must return the same identity, not a fresh one");
}
...
fn a_correct_token_and_valid_init_data_completes_the_link() {
    ...
    let init_data = build_init_data("123:BOT-token", r#"{"id":777,"first_name":"Ann","username":"ann_t"}"#, now());

    let result = complete_link(&pending, &pending.token, &init_data, "123:BOT-token", &identity).unwrap();
...
fn a_wrong_token_is_rejected_even_with_valid_init_data() {
    ...
    let err = complete_link(&pending, "WRONGCODE", &init_data, "123:BOT-token", &identity).unwrap_err();
...
fn an_expired_token_is_rejected_even_with_valid_init_data() {
    ...
    let err = complete_link(&pending, &pending.token, &init_data, "123:BOT-token", &identity).unwrap_err();
...
fn forged_init_data_is_rejected_even_with_the_right_token() {
    ...
    let forged_init_data = build_init_data("999:NOT-the-real-token", r#"{"id":777,"first_name":"Ann"}"#, now());

    let err = complete_link(&pending, &pending.token, &forged_init_data, "123:BOT-token", &identity).unwrap_err();
